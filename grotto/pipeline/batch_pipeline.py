...
class BatchCausalInferencePipeline(BaseCausalInferencePipeline):
    ...
    def _denoise_block(
        ...
        for index, current_timestep in enumerate(self.denoising_step_list):
            timestep = (
                torch.ones([batch_size, current_num_frames], device=self.device, dtype=torch.int64)
                * current_timestep
            )
            ...
            if index < len(self.denoising_step_list) - 1:
                ...
                noisy_input = self.scheduler.add_noise(
                    ...
                    next_timestep
                    * torch.ones(
                        [batch_size * current_num_frames], device=self.device, dtype=torch.long
                    ),
                )
    ...
    def _decode_latent_to_video(
        self, latent: torch.Tensor, vae_cache: List
