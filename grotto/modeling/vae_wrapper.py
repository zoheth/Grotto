...
from grotto.modeling.vae import CacheState, CausalConv3d, VaeDecoder3d
from grotto.modeling.wanx_vae_src import CLIPModel, WanVAE
from grotto.modeling.weight_mapping_config import (
    VAE_DECODER_MAPPING,
    apply_mapping,
    detect_old_vae_format,
)
...
class VaeDecoderWrapper(nn.Module):
    ...
    def forward(self, z: torch.Tensor, *feat_cache: List):
        ...
        is_valid_cache = (
            len(feat_cache) > 0
            and feat_cache[0] is not None
            and isinstance(feat_cache[0], CacheState)
        )
        ...
        if out is None:
            ...
            out = torch.zeros((batch_size, 0, 3, *z.shape[3:]), dtype=z.dtype, device=z.device)
        else:
...
class WanVAEEncoder(nn.Module):
    ...
    def decode(self, latents, device, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        ...
        return self.vae.decode(
            latents, device=device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        )
