...
for i in range(20):  # Enough for most architectures
    VAE_DECODER_MAPPING.update(
        {
            f"decoder.upsamples.{i}.residual.0.gamma": (
                f"decoder.upsamples.{i}.norm1.gamma",
                squeeze_gamma,
            ),
            f"decoder.upsamples.{i}.residual.2.weight": (
                f"decoder.upsamples.{i}.conv1.weight",
                None,
            ),
            f"decoder.upsamples.{i}.residual.2.bias": (f"decoder.upsamples.{i}.conv1.bias", None),
            f"decoder.upsamples.{i}.residual.3.gamma": (
                f"decoder.upsamples.{i}.norm2.gamma",
                squeeze_gamma,
            ),
            f"decoder.upsamples.{i}.residual.6.weight": (
                f"decoder.upsamples.{i}.conv2.weight",
                None,
            ),
            f"decoder.upsamples.{i}.residual.6.bias": (f"decoder.upsamples.{i}.conv2.bias", None),
            f"decoder.upsamples.{i}.shortcut.weight": (
                f"decoder.upsamples.{i}.shortcut.weight",
                None,
            ),
            f"decoder.upsamples.{i}.resample.0.weight": (
                f"decoder.upsamples.{i}.resample.0.weight",
                None,
            ),
            f"decoder.upsamples.{i}.resample.0.bias": (
                f"decoder.upsamples.{i}.resample.0.bias",
                None,
            ),
            f"decoder.upsamples.{i}.resample.1.weight": (
                f"decoder.upsamples.{i}.resample.1.weight",
                None,
            ),
            f"decoder.upsamples.{i}.resample.1.bias": (
                f"decoder.upsamples.{i}.resample.1.bias",
                None,
            ),
            f"decoder.upsamples.{i}.time_conv.weight": (
                f"decoder.upsamples.{i}.time_conv.weight",
                None,
            ),
            f"decoder.upsamples.{i}.time_conv.bias": (
                f"decoder.upsamples.{i}.time_conv.bias",
                None,
            ),
        }
